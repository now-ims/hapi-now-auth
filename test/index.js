...
const jwt =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWQiOjEsImFkbWluIjp0cnVlfQ.RiBQaXCXCwSPwx2B3rsm_Um193HaH55HkyH1uX24UM4';
const jwtAud =
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWUsImF1ZCI6ImFwaS5teWZ1bmFwcC5jb20ifQ.DjPjp8qzQAB3AuzlaIfQ39w9hLVKLqmL4s9i4XdJpyk';
  ...
  server.route([
    {
      ...
      options: { auth: 'jwt_token_type' },
    },
    {
      method: 'GET',
      path: '/jwt_no_keychain',
      handler: defaultHandler,
      options: { auth: 'reject_jwt_no_keychain' },
    },
    {
      method: 'GET',
      path: '/jwt_invalid_keychain',
      handler: defaultHandler,
      options: { auth: 'jwt_invalid_keychain' },
    },
    {
      method: 'GET',
      path: '/jwt_invalid_format',
      handler: defaultHandler,
      options: { auth: 'jwt_invalid_format' },
    },
    {
      method: 'GET',
      path: '/jwt_with_options',
      handler: defaultHandler,
      options: { auth: 'jwt_with_options' },
    },
    {
      method: 'GET',
      path: '/jwt_with_token_type',
      handler: defaultHandler,
      options: { auth: 'jwt_with_token_type' },
    },
    {
      method: 'POST',
      path: '/basic',
    ...
    {
      method: 'POST',
      path: '/basic_default_auth',
      handler: defaultHandler,
      options: {},
    },
    {
      method: 'GET',
      path: '/basic_named_token',
      handler: defaultHandler,
      options: { auth: 'default_named_access_token' },
    },
    {
      method: 'GET',
      path: '/basic_validate_error',
      handler: defaultHandler,
      options: { auth: 'with_error_strategy' },
    },
    {
      method: 'GET',
      path: '/boom_validate_error',
      handler: defaultHandler,
      options: { auth: 'boom_error_strategy' },
    },
    {
      method: 'GET',
      path: '/always_reject',
      handler: defaultHandler,
      options: { auth: 'always_reject' },
    },
    {
      method: 'GET',
      path: '/no_credentials',
      handler: defaultHandler,
      options: { auth: 'no_credentials' },
    },
    {
      method: 'GET',
      path: '/query_token_disabled',
      handler: defaultHandler,
      options: { auth: 'query_token_disabled' },
    },
    {
      method: 'GET',
      path: '/query_token_enabled',
      handler: defaultHandler,
      options: { auth: 'query_token_enabled' },
    },
    {
      method: 'GET',
      path: '/query_token_enabled_renamed',
      handler: defaultHandler,
      options: { auth: 'query_token_enabled_renamed' },
    },
    {
      method: 'GET',
      path: '/cookie_token_disabled',
      handler: defaultHandler,
      options: { auth: 'cookie_token_disabled' },
    },
    {
      method: 'GET',
      path: '/cookie_token_enabled',
      handler: defaultHandler,
      options: { auth: 'cookie_token_enabled' },
    },
    {
      method: 'GET',
      path: '/multiple_headers_enabled',
      handler: defaultHandler,
      options: { auth: 'multiple_headers' },
    },
    {
      method: 'GET',
      path: '/custom_token_type',
      handler: defaultHandler,
      options: { auth: 'custom_token_type' },
    },
    {
      method: 'GET',
      path: '/custom_unauthorized_func',
      handler: defaultHandler,
      options: { auth: 'custom_unauthorized_func' },
    },
    {
      method: 'GET',
      path: '/artifacts',
      handler: defaultHandler,
      options: { auth: 'artifact_test' },
    },
    {
      method: 'GET',
      path: '/chain',
      handler: defaultHandler,
      options: { auth: { strategies: ['reject_with_chain', 'default'] } },
    },
  ]);
...
it('throws when validateFunc is not provided', () => {
  try {
    ...
    expect(e.details[0].message).to.equal(
      '"validate" must be of type function'
    );
  }
});
...
it('returns 200 and success with correct bearer token header set in multiple authorization header', async () => {
  const request = {
    ...
    headers: {
      authorization:
        'TestToken 12345678; FD AF6C74D1-BBB2-4171-8EE3-7BE9356EB018',
    },
  };
...
it('returns 200 and success with correct bearer token header set in multiple places of the authorization header', async () => {
  const request = {
    ...
    headers: {
      authorization:
        'FD AF6C74D1-BBB2-4171-8EE3-7BE9356EB018; TestToken 12345678',
    },
  };
...
it('returns 500 when strategy returns a regular object to validateFunc', async () => {
  const request = {
    method: 'GET',
    url: '/basic_validate_error',
    headers: { authorization: 'Bearer 12345678' },
  };
  const res = await server.inject(request);

  expect(res.statusCode).to.equal(500);
  expect(JSON.stringify(res.result)).to.equal(
...
it('returns a 401 on default auth with access_token query param renamed and set', async () => {
  const requestQueryToken = {
    method: 'GET',
    url: '/basic_named_token?my_access_token=12345678',
  };
...
it('affect header auth and will return 200 and success when specifying custom access_token name', async () => {
  const requestQueryToken = {
    method: 'GET',
    url: '/basic_named_token',
    headers: { my_access_token: 'Bearer 12345678' },
  };
  const res = await server.inject(requestQueryToken);
...
it('allows you to enable auth by query token', async () => {
  const requestQueryToken = {
    method: 'GET',
    url: '/query_token_enabled?authorization=12345678',
  };
  const res = await server.inject(requestQueryToken);
...
it('allows you to enable auth by query token and rename the query param', async () => {
  const requestQueryToken = {
    method: 'GET',
    url: '/query_token_enabled_renamed?my_access_token=12345678',
  };
  const res = await server.inject(requestQueryToken);
...
it('allows you to enable auth by query token and still use header', async () => {
  const requestQueryToken = {
    method: 'GET',
    url: '/query_token_enabled_renamed',
    headers: { my_access_token: 'Bearer 12345678' },
  };
  const res = await server.inject(requestQueryToken);
...
it('allows you to disable auth by query token', async () => {
  const requestHeaderToken = {
    method: 'GET',
    url: '/query_token_disabled?access_token=12345678',
  };
...
it('disables multiple auth headers by default', async () => {
  const request = {
    method: 'POST',
    url: '/basic',
    headers: { authorization: 'RandomAuthHeader 1234; TestToken 12345678' },
  };
  const res = await server.inject(request);
...
it('returns 200 if jwt token is supplied with options', async () => {
  const request = {
    method: 'GET',
    url: '/jwt_with_options',
    headers: { authorization: `Bearer ${jwtAud}` },
  };
...
it('returns 200 if jwt token is supplied with options', async () => {
  const request = {
    method: 'GET',
    url: '/jwt_with_token_type',
    headers: { authorization: `JWT ${jwtAud}` },
  };
