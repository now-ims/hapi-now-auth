    ...
    authenticate: async (request, h) => {
      ...
      if (!authorization) {
        return settings.unauthorized(
          `${settings.accessTokenName} not found in request headers`,
          settings.tokenType
        );
      }
      ...
      if (tokenType.toLowerCase() !== settings.tokenType.toLowerCase()) {
        throw settings.unauthorized(
          `Expected token type ${settings.tokenType} does not match ${tokenType}`,
          settings.tokenType
        );
      }
      ...
      if (settings.verifyJWT) {
        if (settings.keychain.length === 0) {
          return settings.unauthorized(
            `Empty Keychain: Keychain required to validate JWT`,
            settings.tokenType
          );
        }
        ...
        if (token.split('.').length !== 3) {
          return settings.unauthorized(
            `Invalid JWT: Token format not valid`,
            settings.tokenType
          );
        }
        ...
        settings.keychain.some((k) => {
          try {
            ...
            if (error.name === 'TokenExpiredError') {
              throw settings.unauthorized(
                `Invalid JWT: ${error.message} expiredAt: ${error.expiredAt}`
              );
            }
            if (keysTried >= settings.keychain.length) {
              throw settings.unauthorized(
                `Invalid JWT: key or signature is invalid`,
                settings.tokenType
              );
            }
            return false;
      ...
      const { isValid, credentials, artifacts } = await settings.validate(
        request,
        decodedJWT ? { decodedJWT, token } : token,
        h
      );

      if (!isValid) {
        const message =
          settings.allowChaining &&
          request.route.settings.auth.strategies.length > 1
            ? null
            : 'Bad token';
      ...
      if (!credentials || typeof credentials !== 'object') {
        throw h.unauthenticated(
          Boom.badImplementation('Bad token received from auth validation'),
          { credentials, artifacts }
        );
      }
...
exports.plugin = {
  ...
  register: (server, options) =>
    server.auth.scheme('hapi-now-auth', internals.implementation),
};
