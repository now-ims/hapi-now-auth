...
internals.schema = Joi.object().keys({
    ...
    verifyOptions: Joi.object().keys({
        ...
        clickTimestamp: Joi.date().timestamp().optional()
    }).optional(),
...
internals.implementation = (server, options) => {
    ...
    const scheme = {
        authenticate: async (request, h) => {
            ...
            // put logic here for JWT check if true
            if (settings.verifyJWT) {
                ...
                for (let i = 0; i < settings.keychain.length; ++i) {
                    const key = settings.keychain[i];
                    try {
                        var decodedJWT = JWT.verify(token, key, settings.verifyOptions);
                        break;
                    }
                    catch (error) {
                        keysTried++;
            ...
            if (!credentials || typeof credentials !== 'object') {
                throw h.unauthorized(Boom.badImplementation('Bad token received from Bearer auth validation'), { credentials, artifacts });
            }
